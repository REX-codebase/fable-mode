...
25. **Mandatory Red-Team Swarm vs Unit Tests Rule & Remediation Loop Bounding**:
    ...
    - If any breakages occur (`broken_count > 0`), the deliverable MUST be rejected and ping-ponged back for subagent remediation until `verify_red_team_remediation` attests 0 breakages. The `record_breakage_report` and `verify_red_team_remediation` runtime paths MUST persist a dedicated attempt counter and remediation start timestamp. On the fifth failed attempt or after 900 elapsed seconds, whichever occurs first, remediation MUST stop: transition the session to `ESCALATION_UNRESOLVED_BREAKAGES`, record every active breakage in the Epistemic Ledger as `UNKNOWN`/`HYPOTHESIS`, and request human architecture arbitration before saving the session.
...
26. **Mandatory Cortical Evolution & Learning Rule**:
    ...
    - This updates user-profile persistent cortical storage outside the static skill tree (`~/.fable/cortex/<domain>.md` or `DATA_DIR/cortex/`), reinforces synaptic weights ($\Delta W = +0.10$), and synthesizes persistent `HeuristicAntibody` records so past mistakes are permanently eliminated across future agent sessions.
