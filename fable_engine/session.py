...
class FableSession:
    ...
    def __init__(
        ...
        # Remediation limits are durable state, not process-local counters.
        ...
        # This key is local to this session and is only used to authenticate
        # bounded red-team receipts; the receipt never authorizes other actions.
        self._red_team_receipt_key = secrets.token_bytes(32)
        self._reviewed_change_id: Optional[str] = None
        self._clean_report_validated = False
    ...
    def transition_to(self, new_state: Union[SessionState, str], rationale: str = "") -> SessionState:
        ...
        if self.current_state == SessionState.INIT and target_state == SessionState.DEEPTHINK_TIMELOCK:
            ...
            if not self._clean_report_validated:
                raise ValueError("Transition from ARBITRATION to SEALED requires a validated clean red-team report.")
            ...
            if not self._clean_report_validated:
                raise ValueError("Transition from REMEDIATION_REQUIRED to SEALED requires a validated clean red-team report.")
            if len(self.active_breakages) > 0:
    ...
    def set_goal_rubric(
        ...
        for idx, item in enumerate(raw_items):
            ...
            evidence_type = str((item.get("evidence_type") if isinstance(item, dict) else "") or meta.get("evidence_type") or ("verifier" if verifier else "")).strip()
            evidence_phase = str((item.get("evidence_phase") if isinstance(item, dict) else "") or meta.get("evidence_phase") or (self.active_phase if verifier else "")).strip()
            parsed_item = {
                "pointer_id": p_id,
                "description": desc,
            ...
            # Rubric registration is intentionally non-gating.  Callers may
            # submit a claimed score before a typed, current-stage proof is
            # available; retain it as an explicitly untrusted score and let
            # the clean sealing gate validate it later.
        ...
        has_untrusted_items = any(
            bool(it.get("satisfied")) and (it.get("metadata") or {}).get("evidence_trust") == "untrusted"
            for it in parsed_items
        )
        status = (
            "achieved" if current_score >= target_score_val and not has_untrusted_items
            else "pending"
        )
    ...
    def evaluate_goal_rubric(
            ...
            for it in candidate["items"]:
                if it.get("pointer_id") == p_id:
                    ...
                    if "verifier_command" in ev:
                        ...
                        # A verifier supplied during evaluation is typed
                        # current-stage evidence unless the caller provides a
                        # more specific type/phase below.
        ...
        for item in candidate["items"]:
            # Evaluation updates are also non-gating.  A score can be recorded
            # now, but an unverified satisfied item must remain untrusted until
            # _validate_clean_stage_evidence runs for a sealing attempt.
            self._annotate_rubric_item_trust(item)
        total_weight = sum(it.get("weight", 1.0) for it in candidate["items"])
        if total_weight > 0:
            weighted_sum = sum(float(it.get("score", 1.0 if it.get("satisfied") else 0.0)) * float(it.get("weight", 1.0)) for it in candidate["items"])
            current_score = round(weighted_sum / total_weight, 4)
        else:
            current_score = 0.0
        ...
        candidate["status"] = (
            "achieved" if current_score >= target_score and not has_untrusted_items
            else "in_progress"
        )
        candidate["last_evaluated_at"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(self._wall_clock()))
        rubric.clear()
    ...
    def _attack_vector_results(self, report_data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """Return a complete, deterministic five-vector result map for a report."""
        supplied = report_data.get("attack_vector_results")
        if isinstance(supplied, dict):
            return {str(k): dict(v) for k, v in supplied.items() if isinstance(v, dict)}
    ...
    def issue_red_team_receipt(
        self, report_data: Dict[str, Any], reviewed_change_id: str, origin: str = "red_team_swarm"
    ) -> Dict[str, Any]:
        """Issue a short-lived local HMAC attestation for a complete clean review.

        The receipt is deliberately bounded to this session, one report, one immutable
        reviewed-change identifier, and five explicitly named attack vectors.
        """
        if not isinstance(report_data, dict) or int(report_data.get("broken_count", 0)) != 0:
        ...
        if self._reviewed_change_id is not None and not hmac.compare_digest(self._reviewed_change_id, change_id):
            raise ValueError("The reviewed_change_id is immutable for this session.")
        vectors = self._attack_vector_results(report_data)
        if set(vectors) != set(RED_TEAM_ATTACK_VECTORS) or any(not bool(v.get("passed")) for v in vectors.values()):
            raise ValueError("A clean receipt requires results for all five attack vectors.")
        ...
        receipt = {
            ...
            "report_digest": hashlib.sha256(self._canonical_receipt_payload({
                k: v for k, v in report_data.items() if k not in ("red_team_receipt", "receipt", "attestation")
            })).hexdigest(),
        }
        body = self._canonical_receipt_payload(receipt)
        receipt["signature"] = hmac.new(self._red_team_receipt_key, body, hashlib.sha256).hexdigest()
        return receipt

    def _validate_red_team_receipt(self, report_data: Dict[str, Any]) -> None:
        receipt = report_data.get("red_team_receipt") or report_data.get("receipt") or report_data.get("attestation")
        if not isinstance(receipt, dict):
            raise ValueError("Zero-breakage reports require an authenticated red-team receipt.")
        required = ("receipt_type", "receipt_id", "session_id", "report_id", "report_origin",
                    "reviewed_change_id", "attack_vector_results", "issued_at", "expires_at",
                    "report_digest", "signature")
        if any(not receipt.get(k) and k not in ("issued_at", "expires_at") for k in required):
        ...
        try:
            ...
            if float(receipt["issued_at"]) > now or float(receipt["expires_at"]) < now:
                raise ValueError("Red-team receipt is expired or not yet valid.")
            if float(receipt["expires_at"]) - float(receipt["issued_at"]) > MAX_REMEDIATION_SECONDS:
                raise ValueError("Red-team receipt validity exceeds the bounded limit.")
        except (TypeError, ValueError) as exc:
            if isinstance(exc, ValueError) and str(exc).startswith("Red-team receipt"):
                raise
            raise ValueError("Red-team receipt has invalid validity bounds.") from exc
        ...
        if any(not isinstance(v, dict) or v.get("passed") is not True for v in vectors.values()):
            raise ValueError("Red-team receipt does not attest five passing attack vectors.")
        report_vectors = self._attack_vector_results(report_data)
        if report_vectors != vectors:
        ...
        expected = hmac.new(self._red_team_receipt_key, self._canonical_receipt_payload(unsigned), hashlib.sha256).hexdigest()
        if not hmac.compare_digest(expected, str(receipt.get("signature", ""))):
            raise ValueError("Red-team receipt authentication failed.")
        expected_digest = hashlib.sha256(self._canonical_receipt_payload({
            k: v for k, v in report_data.items() if k not in ("red_team_receipt", "receipt", "attestation")
        })).hexdigest()
        ...
        if str(report_data.get("reviewed_change_id", "")).strip() != str(receipt.get("reviewed_change_id", "")).strip():
            raise ValueError("Red-team receipt reviewed-change binding failed.")
        if self._reviewed_change_id is not None and not hmac.compare_digest(self._reviewed_change_id, str(receipt.get("reviewed_change_id", "")).strip()):
            raise ValueError("The reviewed_change_id is immutable for this session.")

    def _fresh_stage_records(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
        phase = self.active_phase
        fresh_epistemic = [i for i in self.epistemic_ledger if not i.get("_restored_untrusted") and i.get("phase") == phase]
        fresh_refinements = [r for r in self.refinement_cycles if not r.get("_restored_untrusted") and r.get("phase") == phase]
        fresh_files = [f for f in self.file_changes if not f.get("_restored_untrusted") and f.get("phase") == phase]
        fresh_rubrics = [r for r in self.goal_rubrics if not r.get("_restored_untrusted") and r.get("created_phase", phase) == phase]
        return fresh_epistemic, fresh_refinements, fresh_files, fresh_rubrics

    def derive_reviewed_change_id(self) -> str:
        """Return a deterministic ID for fresh current-stage file changes.

        A clean remediation must review an actual current-stage change.  The
        derived ID is stable for the exact change records and intentionally
        excludes restored/untrusted history.
        """
        fresh_files = self._fresh_stage_records()[2]
        if not fresh_files:
            return ""
        payload = json.dumps(fresh_files, sort_keys=True, separators=(",", ":"))
    ...
    def _validate_satisfied_rubric_item(self, item: Dict[str, Any]) -> None:
        ...
        evidence_type = str(item.get("evidence_type") or meta.get("evidence_type") or "").strip()
        evidence_phase = str(item.get("evidence_phase") or meta.get("evidence_phase") or "").strip()
        if not evidence_type or evidence_phase != self.active_phase:
            raise ValueError("Satisfied rubric criteria require typed evidence from the current stage.")
        verifier = str(item.get("verifier_command", "")).strip()
        receipt_id = str(item.get("evidence_receipt_id", "")).strip()
        valid_receipt = any(isinstance(r, dict) and str(r.get("receipt_id", "")) == receipt_id for r in self.proof_receipts)
    ...
    def _annotate_rubric_item_trust(self, item: Dict[str, Any]) -> bool:
        """Record whether a claimed satisfied item is ready for a clean seal.

        Registration/evaluation are data-ingestion operations and must not be
        confused with the sealing authority.  Preserve claimed scores for
        feedback, but mark claims that lack current typed evidence so they
        cannot silently satisfy the clean-stage gate.
        """
    ...
    def _validate_clean_stage_evidence(self) -> None:
        ...
        proven = [i for i in epistemic if i.get("tag") == "PROVEN" and str(i.get("evidence", "")).strip()]
        if len(proven) < 2 or not refinements or not files:
            raise ValueError("Clean report lacks fresh current-stage epistemic, refinement, and file-change evidence.")
        achieved = False
        for rubric in rubrics:
            if rubric.get("status") == "achieved" or float(rubric.get("current_score", 0.0)) >= float(rubric.get("target_score", 0.95)):
                ...
                break
        if not achieved:
            raise ValueError("Clean report lacks an achieved fresh rubric with validated criteria evidence.")
    ...
    def _validate_breakage_report(self, report_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            raise ValueError("Breakage report counts must be integers.") from exc
        if broken_count < 0 or total_probes < len(findings) or broken_count != sum(
            1 for f in findings if bool(f.get("broken", False) if isinstance(f, dict) else getattr(f, "broken", False))
        ):
        ...
        if broken_count == 0:
            if report_data.get("report_origin") != "red_team_swarm":
                raise ValueError("Zero-breakage reports must identify the Red-Team origin.")
    ...
    def record_breakage_report(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate then atomically record a report and advance remediation state."""
        # Do every trust and stage check before mutating reports or active findings.
        broken_count = self._validate_breakage_report(report_data)
        now = self._wall_clock()
        if broken_count == 0:
            if self.current_state not in (SessionState.RED_TEAM_GATE, SessionState.ARBITRATION, SessionState.REMEDIATION_REQUIRED):
            ...
            self._clean_report_validated = True
        ...
        findings = report_data.get("findings", [])
        # Initial and subsequent failed reports each consume one durable attempt.
        ...
        active = []
        for f in findings:
            if bool(f.get("broken", False) if isinstance(f, dict) else getattr(f, "broken", False)):
                active.append({
        ...
        if attempt >= MAX_REMEDIATION_ATTEMPTS or elapsed >= MAX_REMEDIATION_SECONDS:
            for finding in active:
                finding["epistemic_status"] = "UNKNOWN" if not finding.get("hypothesis") else "HYPOTHESIS"
                finding["human_arbitration_required"] = True
        # All validation above succeeded: publish the durable mutation atomically.
        self.breakage_reports.append(dict(report_data))
        ...
        if attempt >= MAX_REMEDIATION_ATTEMPTS or elapsed >= MAX_REMEDIATION_SECONDS:
            self.transition_to(SessionState.ESCALATION_UNRESOLVED_BREAKAGES, "Remediation bounds exceeded; human arbitration required")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "remediation_attempt_count": self.remediation_attempt_count,
            "remediation_started_at": self.remediation_started_at,
            "red_team_receipt_key": self._red_team_receipt_key.hex(),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "FableSession":
        ...
        # SEALED/EVOLVED is an authority decision, not portable session data.
        # A tampered or stale persistence file must never turn directly into a
        # seal/evolution bypass; a fresh run must repeat the authenticated gate.
        if restored_state in (SessionState.SEALED, SessionState.EVOLVED):
            restored_state = SessionState.INIT
        session.current_state = restored_state
        session._clean_report_validated = False
        ...
        # Keep receipt verification possible across a normal process reload; restored
        # records remain individually untrusted and cannot satisfy fresh gates.
        try:
            stored_key = str(data.get("red_team_receipt_key", ""))
            if stored_key:
                session._red_team_receipt_key = bytes.fromhex(stored_key)
        except (TypeError, ValueError):
            session._red_team_receipt_key = secrets.token_bytes(32)
        ...
        session.file_changes = [dict(item, _restored_untrusted=True) for item in data.get("file_changes", []) if isinstance(item, dict)]
        ...
        session.goal_rubrics = [dict(item, _restored_untrusted=True) for item in data.get("goal_rubrics", []) if isinstance(item, dict)]
