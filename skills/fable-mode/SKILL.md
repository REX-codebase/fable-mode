...
description: >-
  ...
  Fleet (fable_v2.coder_fleet), Zero-Cost Multi-Source Research Scraping Engine (`scrape_web`, `scrape_youtube`,
  `scrape_reddit`, `scrape_x`, `scrape_github`, `scrape_arxiv`), and dedicated Domain Cognitive Gears (including domain: "design" /
...
## Non-Frontier AI Mandatory Step-by-Step Execution Protocol
...
7. **AAA Three.js / WebGPU Game-Grade Standards**:
   - **Architectural Paradigm Distinction**: Strictly delineate **Vanilla Three.js (Imperative)** from **React Three Fiber (Declarative R3F)**:
     * *Vanilla Three.js*: Manage lifecycle imperatively via explicit `requestAnimationFrame`, `ResizeObserver`, and manual traversal teardown; use `WebGLRenderer` + `EffectComposer` for universal compatibility, or `WebGPURenderer` (awaited `init()`) + TSL `PostProcessing` for next-gen compute. Never mix backends.
     * *React Three Fiber (R3F)*: Declarative scene graph inside `<Canvas>`; wrap asynchronous asset loaders (`useGLTF`) in `<Suspense>`; **NEVER** call `useState` or parent state dispatch inside `useFrame()` (mutate object refs directly to prevent catastrophic render-loop thrashing).
   - **The 5-Point Scene Grounding Contract**: Every 3D scene must satisfy five non-negotiable invariants before rendering to prevent black-screen void failures:
     1. *Container*: Resilient dimension fallbacks (`clientWidth || innerWidth`) with `ResizeObserver` observing size deltas.
     2. *Camera*: Positioned outside the geometry bounding volume (e.g. `(0, 3, 8)` looking at `(0, 0, 0)` or auto-framed from bounding sphere radius).
     3. *Lighting*: Mandatory PBR baseline illumination: `AmbientLight(0xffffff, 0.6)` + `DirectionalLight(0xffffff, 1.8)` at `(5, 10, 7)`.
     4. *ColorSpace*: Output set to `THREE.SRGBColorSpace`; diffuse/color maps set to `SRGBColorSpace`; normal/roughness/metalness/AO data maps set to `NoColorSpace`.
     5. *Teardown*: Universal recursive disposal of geometries, materials, all texture slots, buffer attributes, and `renderer.forceContextLoss()`.
   - **Fidelity Scaling Guardrail**: Scale fidelity to task scope—do not over-engineer simple 3D UI badges, hero cards, or subtle background spheres into complex multi-pass bloom/GTAO game engines when lightweight standard materials and simple render loops achieve the design objective with zero overhead.
...
20. **Modular Fable Part 2: Dynamic Customizable Cortical Lobe Sprouting & BCM Plasticity (`HebbianPlasticityEngine`)**: Lifelong neuro-evolutionary adaptation replacing static vector RAG with living, customizable cortical domain lobes. Strictly avoids hardcoded domain constraints: dynamically sprouts, names, describes, and evolves custom lobes from scratch for any language, framework, or scientific discipline (e.g. `zig_systems`, `mojo_kernels`, `solidity_evm`, `bioinformatics`) alongside the 5 foundational lobes (`rust`, `python`, `design_3d`, `research`, `concurrency`). Updates synaptic weights via directional BCM / STDP plasticity (LTP on success $\Delta W = +\eta \cdot A_{domain} \cdot A_{j}$, LTD on failure $\Delta W = -\lambda \cdot A_{domain} \cdot A_{j}$) with continuous activity signals, homeostatic normalization bounded in $[0.05, 1.0]$, automated closed-loop RedTeam-to-Cortex consolidation, and immunological `HeuristicAntibody` synthesis.
...
25. **Mandatory Red-Team Swarm vs Unit Tests Rule & Remediation Bounding**: For non-frontier AI models, standard unit tests check basic happy-path assumptions and **do not possess thinking or counterfactual analysis ability**. Non-frontier AI models **MUST NEVER** declare a task finished or stop merely because unit tests pass and **MUST** invoke `RedTeamSwarm` (`red_team_code_review` / `verify_red_team_remediation`) across 5 attack vectors. Runtime remediation stops at the fifth failed attempt or after 900 elapsed seconds and transitions to `ESCALATION_UNRESOLVED_BREAKAGES` for human architecture arbitration.
26. **Mandatory Cortical Evolution & Consolidation**: After milestone verification, non-frontier AI models **MUST** invoke `evolve_cortex` / `cortical_consolidate_task` to update user-profile persistent cortical storage outside the static skill tree (`~/.fable/cortex/<domain>.md` or `DATA_DIR/cortex/`), potentiate synaptic weights, and synthesize `HeuristicAntibody` records so past mistakes are permanently eliminated across future sessions.
...
- [Anti-Slop Frontend Architecture & Awwwards Design System](./references/anti-slop-frontend-architecture.md) — Brief inference ("Read the Room"), the Three Dials (Variance, Motion, Density), the 6 Haute Archetypes, 7-layer optical depth staging, fluid clamp typography math, and the 5-point Pre-Flight Design Quality Gate.
...
- [Modular Fable Part 2: Hebbian Cortical Plasticity & Dynamic Lobe Sprouting Reference](./references/hebbian-cortical-plasticity.md) — Open-ended customizable cortical sprouting, BCM directional plasticity ($\Delta W_{ij}$ LTP/LTD), continuous activity signals, homeostatic synaptic scaling, automated closed-loop RedTeam consolidation, immunological `HeuristicAntibody` synthesis, dynamic lobe discovery, and prompt recall injection.
