...
8. **`EVOLVED`**: Triggered by `evolve_cortex`; consolidates task lessons into `DATA_DIR/cortex/<domain>.md`, potentiates synaptic weights ($\Delta W = +0.10 \cdot A_{\text{domain}} \cdot A_{\text{node}}$), and synthesizes permanent antibodies (`ab_<domain>_<scenario_id>`).
