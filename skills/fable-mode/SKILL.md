...
25. **Mandatory Red-Team Swarm vs Unit Tests Rule & Remediation Bounding**: Standard unit tests check basic happy-path assumptions and **do not possess thinking or counterfactual analysis ability**. The AI **MUST NEVER** declare a task finished or stop merely because unit tests pass. The AI **MUST** invoke `RedTeamSwarm` (`red_team_code_review` / `verify_red_team_remediation`) across 5 attack vectors and resolve all breakages before milestone completion, capped at a maximum of 5 remediation iterations or 15 elapsed minutes (triggering `ESCALATION_UNRESOLVED_BREAKAGES` if unresolved).
